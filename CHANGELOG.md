...
- 优化升级[备份还原](./apps/BackupRestore.js)，支持自定义备份cron、自定义备份各插件的文件目录、自定义额外备份的文件、自定义备份保留时长，优化备份逻辑

#### 3-27

**ver 2.6.1**

- `查看、删除、上传`[随机老婆](./apps/randomWife.js)和[水群统计背景](./apps/sqtj.js)的指令必须使用前缀，避免误触
- 修复序号选择无效
- [备份还原](./apps/BackupRestore.js)同时备份还原example
- 优化查看图片展示效果，同时展示图片名称
- [锁定功能](./apps/Event/Lockdown.js)支持按照key搜索（插件名）
...
- 为适配[水群统计](./apps/sqtj.js)的优化等，获取的聊天记录直接保存本地，不再提供开关
...
- 修复[放大图片](./apps/bigjpg.js)可能的放大失败
...
- 优化[设置图](./system/cfg.js)展示，仅以全局设置为准的设置不再于群设置中展示，优化对只查看某一类设置模块的支持
...
- 优化[水群统计](./apps/sqtj.js)图片展示效果，支持自行管理水群统计背景，支持自定义排名人数
- 优化上下文序号选择逻辑，支持发送`all`便捷全部选中
- 优化UC插件报错，使自身app的报错全部走UC自身的log.error
- 优化[伪装](./apps/camouflage.js)引用回复逻辑
