...
- 受图片展示限制，设置图中部分设置不会进行完整展示，建议在 **锅巴 → 插件管理 → UC-plugin → 设置** 进行全局设置的查看和更改
...
1. Q：单独群设置是如何生效的？
  - A: 在不存在 **单独群设置文件** 的群内，[群设置](#按照生效范围分类)会采用当前的[全局设置](#按照生效范围分类)；而在拥有 **单独群设置文件** 的群内，群设置会使用该文件中的数据，即 **群设置优先于全局设置**（除[被锁定的设置](#锁定设置)和少数只以全局设置为准的设置以外）。

2. Q：单独群设置文件如何生成的？
  - A: 当[单独修改某群的设置](#按照生效范围分类)时（包括设置主人等），系统会自动生成该群的群设置文件，此群设置文件会以 **当时的全局设置** 为初始值，新增群设置文件后会立即生效，此后全局设置与此群设置基本无关，修改全局设置也不会影响该群设置（除[被锁定的设置](#锁定设置)和少数只以全局设置为准的设置以外）。

3. Q：锁定设置是如何实现的？
  - A：当UC的插件读取当前群设置时，被锁定的部分设置会覆盖群设置同名项，以此实现锁定设置的目的，此过程只发生在内存中而不会修改群设置文件。因此锁定设置后，仍可修改群内的被锁定的设置，只是修改的值不会生效，直到解除对该设置的锁定才会重新生效。

4. Q：如何判断某群是否有单独群设置？
  - A：法一：进入UC根目录config/groupCfg，文件夹内的即为单独群配置。
  -   法二：查看该群任一UC设置（可使用#UC设置+群号指定查看），查看最上方的标题，若标题最后有一个 **星号 \*** ，则代表此群不存在单独群设置（当前查看的群设置是根据全局设置生成）
...
5. Q：我已经开启了XXX功能，为什么还是无法使用？
  - A：如果你已经阅读并理解了上述四个Q&A，那么你应该能自主解决此问题。

6. Q：我已经开启了XXX功能，为什么 **其他人** 无法使用？
